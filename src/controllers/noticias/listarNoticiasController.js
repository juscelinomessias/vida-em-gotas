...
const listarNoticiasWeb = (req, res) => {
    res.sendFile(path.join(__dirname, '../../public/admin/admin_view_noticias', 'listar_noticias.html'));
};
...
const listarNoticiasVideoController = async (req, res) => {
    try {
        ...
        const orientacoes = result.rows.map(noticia => {
            if (noticia.imagens) {
